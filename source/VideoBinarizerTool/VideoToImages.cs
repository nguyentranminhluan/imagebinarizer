...
namespace VideoBinarizerTool
{
    public class VideoToImages
    {
        ...
        public void VidToImage(BinarizerParams config)
        {
            this.config = config;
            //
            ...
            {
                if (frameNum % config.FrameFilter == 0)
                {
                    //imageData.ToBitmap().Save($"{framesPath}\\{frameNum}.png");
                    Bitmap b = new Bitmap(config.ImageWidth, config.ImageHeight);
                    Graphics g = Graphics.FromImage((System.Drawing.Image)b);
                    g.DrawImage(imageData.ToBitmap(), 0, 0, config.ImageWidth, config.ImageHeight);
                    g.Dispose();
